...
for i=1:n
  ...
  switch l.type
    ...
    case 'pointweight'
      res(i+1).x = nnpointweight(res(i).x, l.weights{1}) ;

    case 'l2normloss'
      res(i+1).x = vl_l2normloss(res(i).x,l.class,l.Beta);
...
if doder
  ...
  for i=n:-1:backPropLim
    ...
    case 'pointweight'
      [res(i).dzdx, dzdw{1}] = nnpointweight(res(i).x, l.weights{1}, res(i+1).dzdx) ;

    case 'l2normloss'
      [res(i).dzdx] = vl_l2normloss(res(i).x,l.class,l.Beta,res(i+1).dzdx);
    ...
    switch l.type
      case {'conv', 'convt', 'bnorm', 'pointweight'}
        if ~opts.accumulate
          res(i).dzdw = dzdw ;
        else
          for j=1:numel(dzdw)
